...
public interface Response {
  ...
  //   * If none of write(), body(), and bodyAsBytes() have been called and Route.handle returns null,
  ...
  // Enable streaming mode for responses where multiple write() calls will be made.
  // In streaming mode, Content-Length is NOT sent, and the client reads until connection closes.
  // Call this BEFORE the first write() if you plan to stream data in chunks.
  // Default is false (Content-Length is sent based on first write for backward compatibility).
  default void setStreamingMode(boolean streaming) {
    // Default implementation does nothing - for backward compatibility with existing implementations
  }
