...
public interface FlameRDD {
  ...
  /**
   * Lambda interface with context for batched KVS operations.
   * Use this when your lambda needs to do KVS writes that should be batched.
   */
  public interface StringToIterableWithContext extends Serializable {
    /**
     * @param value The input value
     * @param kvsReader KVSClient for reads (not batched)
     * @param batchedWriter BatchedKVSClient for writes (pooled and batched across all threads)
     * @return Iterable of output strings
     */
    Iterable<String> op(String value, KVSClient kvsReader, BatchedKVSClient batchedWriter) throws Exception;
  };
  ...
  /**
   * Lambda interface with context for batched KVS operations in mapToPair.
   */
  public interface StringToPairWithContext extends Serializable {
    FlamePair op(String value, KVSClient kvsReader, BatchedKVSClient batchedWriter) throws Exception;
  };
   ...
   */
  public interface StringToPairIterableWithContext extends Serializable {
    Iterable<FlamePair> op(String value, KVSClient kvsReader, BatchedKVSClient batchedWriter) throws Exception;
  };
  ...
  /**
   * Lambda interface with context for batched KVS operations in filter.
   */
  public interface StringToBooleanWithContext extends Serializable {
    boolean op(String value, KVSClient kvsReader, BatchedKVSClient batchedWriter) throws Exception;
  }
  ...
  // Parallel version of distinct with batched I/O for better throughput.
  ...
  // Parallel version of mapToPair with batched I/O and context; uses bounded queues and
  // worker-grouped batching for better throughput on CPU-heavy lambdas.
  // Provides shared BatchedKVSClient for internal writes pooled across all threads.
  ...
  // Parallel version of filter with batched I/O and context; uses bounded queues and
  // worker-grouped batching for better throughput on CPU-heavy predicates.
  // Provides shared BatchedKVSClient for internal writes pooled across all threads.
