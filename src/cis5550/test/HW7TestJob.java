...
public class HW7TestJob {
    ...
    if (args[0].equals("count")) {
			...
			for (int i=1; i<args.length; i++)
				list.add(args[i]);
			List<FlamePair> out = ctx.parallelize(list).flatMapToPair(s -> Arrays.asList(new FlamePair[] { new FlamePair(""+s.charAt(0), s.substring(1)) })).collect();
			Collections.sort(out);
			String ret = "";
			for (FlamePair p : out) 
			...
			for (int i=1; i<args.length; i++)
				list.add(args[i]);
			List<String> out = ctx.parallelize(list).filter(s -> Integer.valueOf(s) > 5).collect();
			Collections.sort(out);
			String ret = "";
			for (String s : out)
				ret = ret + (ret.equals("") ? "" : ",") + s;
			ctx.output(ret);
    } else if (args[0].equals("mapPartitions")) {
			LinkedList<String> list = new LinkedList<String>();
			for (int i=1; i<args.length; i++)
				list.add(args[i]);
			List<String> out = ctx.parallelize(list).mapPartitions(iter -> {
				List<String> partition = new ArrayList<String>();
				while (iter.hasNext())
					partition.add(iter.next());
				List<String> result = new ArrayList<String>();
				result.add("count:" + partition.size());
			...
			LinkedList<String> list1 = new LinkedList<String>();
			LinkedList<String> list2 = new LinkedList<String>();
			int numPairs1 = Integer.valueOf(args[1]);
			int numPairs2 = Integer.valueOf(args[2]);
			for (int i=0; i<numPairs1*2; i+=2)
				list1.add(args[3+i]+","+args[4+i]);
			for (int i=0; i<numPairs2*2; i+=2)
				list2.add(args[3+numPairs1*2+i]+","+args[4+numPairs1*2+i]);
			FlamePairRDD rdd1 = ctx.parallelize(list1).mapToPair(s -> new FlamePair(s.split(",")[0], s.split(",")[1]));
			FlamePairRDD rdd2 = ctx.parallelize(list2).mapToPair(s -> new FlamePair(s.split(",")[0], s.split(",")[1]));
