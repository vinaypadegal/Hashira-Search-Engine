#!/bin/bash
# Run PageRank computation on AWS
# Usage: ./scripts/run-pagerank-aws.sh [threshold] [percentage] [terraform_dir]
...
#
# Example:
#   ./scripts/run-pagerank-aws.sh                    # Uses defaults: 0.01, 100%, terraform
#   ./scripts/run-pagerank-aws.sh 0.001              # Custom threshold, default percentage
#   ./scripts/run-pagerank-aws.sh 0.001 95          # Custom threshold and percentage
#   ./scripts/run-pagerank-aws.sh 0.001 95 terraform-indexer  # All custom
...
echo "==================================="
echo "Building and Running PageRank (Optimized)"
echo "==================================="
echo "Convergence threshold: $THRESHOLD"
echo "Convergence percentage: $PERCENTAGE%"
echo "Using PageRankOptimized class"
echo ""
...
echo "Submitting PageRank job to $FLAME_ENDPOINT ..."
echo "  Threshold: $THRESHOLD"
echo "  Percentage: $PERCENTAGE%"
echo ""

result=$(java -cp bin cis5550.flame.FlameSubmit "$FLAME_ENDPOINT" "$JAR_FILE" cis5550.jobs.PageRankOptimized "$THRESHOLD" "$PERCENTAGE" 2>&1)
echo "$result"
echo ""
